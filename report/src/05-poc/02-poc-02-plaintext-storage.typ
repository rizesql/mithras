...
creat prin exploatarea VULN-01). Se presupune că atacatorul a obținut acces la mediul de
persistență printr-o breșă secundară (simulată aici printr-un client PostgreSQL direct).
...
*Rezultat observat:*
Interogarea returnează imediat înregistrările, expunând parola originală sub formă de text
simplu (_plaintext_) în coloana `secret`. Atacatorul nu mai trebuie să depună niciun efort
computațional pentru a obține accesul la cont, informația fiind extrasă direct.

Dacă s-ar folosi implementarea corectă, coloana `secret` ar afișa un șir lung ce începe cu
`$argon2id$`, protejat criptografic prin derivare de cheie.
