...
gestiune a identității, fiind adesea veriga slabă exploatată pentru preluarea
conturilor (*Account Takeover*). Mithras implementează acest flux urmând riguros
recomandările *OWASP Forgot Password Cheat Sheet*, asigurând un echilibru între
securitatea criptografică și protecția împotriva scurgerilor de informații.
...
compozit pentru token-ul transmis utilizatorului, structurat sub forma
`{ID}.{Secret}`. Identificatorul public (`ID`) este un *NanoID* (de exemplu, cu
prefixul `rst_`), un format opac, url-friendly și rezistent la atacuri de tip
enumerare.

Această separare între identificator și materialul criptografic secret permite
localizarea rapidă a înregistrării în baza de date fără a expune materialul
sensibil în log-uri de acces sau la nivelul stratului de stocare. În timp ce
identificatorul NanoID este utilizat pentru regăsirea datelor, partea de `Secret`
este utilizată exclusiv pentru verificarea integrității cererii, fiind stocată sub
formă de hash.
...
înainte de persistență. Astfel, chiar și în cazul unei scurgeri de date, atacatorul
nu poate reconstitui materialul necesar pentru a finaliza o resetare neautorizată.

=== Prevenirea Enumerării

O măsură critică de securitate implementată la nivelul endpoint-ului de solicitare
este returnarea unui răspuns uniform de succes indiferent dacă adresa de email există sau nu
în sistem. Această tactică elimină capacitatea unui atacator de a utiliza formularul de
resetare pentru a confirma existența conturilor legitime în baza de date.

=== Validarea și Constrângerile de Securitate

Procesul de resetare finalizat prin `POST /reset-password` impune o serie de
constrângeri riguroase. Fiecare token este de unică folosință, marcat ca utilizat
atomic după prima resetare reușită, și are o durată de viață limitată la o oră. În
momentul resetării, sistemul invalidează automat orice alte cereri de resetare
...
garantând că orice potențial atacator care deține un token de sesiune valid trebuie să
se reautentifice. Această măsură asigură controlul exclusiv al proprietarului asupra contului.
