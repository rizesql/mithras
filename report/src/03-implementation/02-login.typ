...
verificare, asigurând un răspuns uniform către utilizator indiferent de natura
eșecului.

Pentru a asigura un nivel ridicat de protecție, procesul de autentificare urmează o
secvență strictă de validări, de la rate limiting la nivel de rețea până la
hashing-ul constant-time la nivel de aplicație.
...
O vulnerabilitate comună în sistemele de autentificare este diferența măsurabilă în
timpul de răspuns între un email inexistent și o parolă greșită. Mithras neutralizează
...
cu cel stocat folosind `subtle.ConstantTimeCompare` din biblioteca standard Go. Această
funcție asigură că timpul de execuție al comparației nu depinde de numărul de
octeți care coincid, prevenind atacurile de tip timing care ar putea dezvălui
fragmentar conținutul hash-ului.
...
indisponibilă, serverul refuză cererea de autentificare (HTTP 500/503). Această
abordare este preferată în locul unei degradări "permisive" (*Fail-Open*), deoarece
absența controalelor de securitate ar lăsa sistemul complet vulnerabil la atacuri de
forță brută în perioadele de instabilitate a infrastructurii.
...
middleware folosind un backend Redis, care protejează împotriva inundării serverului
cu cereri. A doua barieră este *Account Lockout-ul*, prin care, după 5 tentative de
autentificare eșuate, contul este blocat automat pentru o perioadă de 15 minute.
Această stare este persistentă în baza de date (coloana `locked_until`), fiind
verificată înaintea oricărei procesări de parolă.
...
informații care ar putea fi utilizat pentru *User Enumeration*, asigurând că un
atacator nu poate distinge între diversele cauze de eșec ale autentificării.
