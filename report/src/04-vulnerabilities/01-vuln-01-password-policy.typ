...
(versiunea V1) acceptă orice șir de caractere non-gol, validările pentru lungimea minimă și
complexitate fiind complet eliminate. Defectul este clasificat conform OWASP Top 10 ca
A07:2025 (Identification and Authentication Failures) și CWE-521 (Weak Password Requirements) @cwe521.

=== Fundamentare Teoretică și Implementare Defectuoasă

O politică de parole robustă reprezintă primul strat de apărare împotriva preluării conturilor.
În absența impunerii unei entropii minime (lungime și varietate a setului de caractere),
utilizatorii au tendința de a alege parole scurte, previzibile sau derivate din informații
personale.
...
utiliza dicționare comune (precum `rockyou.txt`) pentru a efectua încercări de autentificare.
Sistemul va accepta la înregistrare parole de un singur caracter, creând o vulnerabilitate
exploatabilă ulterior la nivelul endpoint-ului de login.
